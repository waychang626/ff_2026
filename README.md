...
1. **Morning of.** `ffdraft check --league <id> --seat <n>`. It must print
   `READY`. Confirm the starters, the playoff structure and the baselines are
   what the league actually uses.
2. **Sanity-read the board.** `ffdraft board --league <id> --top 30`. If a name
   near the top is wrong, the projections are wrong, and no amount of simulation
   fixes that.
3. **Draft.** `ffdraft draft --league <id> --seat <n>`. Type every pick as it
   happens — including everyone else's. The recommendation appears automatically
   when you are on the clock. `?` lists the commands.
4. **Afterwards.** The pick log in `logs/` is the first backtest dataset and the
...
|---|---|---|
| Teams | 8 | 12 |
| Starters | QB WR WR WR RB RB W/R/T W/R/T **Q/W/R/T** K DEF | QB RB RB WR WR TE W/R/T K DEF |
| Bench / IR | 6 / 2 | 4 / 1 |
| Draft | 17 rounds, 136 picks | 13 rounds, 156 picks |
| Playoffs | 4 of 8, weeks 16–17 | 6 of 12, weeks 15–17 |
| Baselines | QB 17 · RB 23 · WR 33 · TE 10 · K 9 · DST 9 | QB 13 · RB 30 · WR 31 · TE 14 · K 13 · DST 13 |
...
- **League 1 has no TE slot and a superflex.** A tight end only plays by beating
  a flex-eligible RB/WR, and with 4-pt passing TDs a second QB beats a
  flex-level WR comfortably. Assume all 8 teams start two QBs.
- **WR replacement is *deeper in the smaller league*.** League 1 requires three
  WRs and two flexes (32 WRs start); League 2 requires two and one (30 start).
  Roster shape beats league size, and the two boards are not related by a simple
  shift. See `docs/baselines.md`.
